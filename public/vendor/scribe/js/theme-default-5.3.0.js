document.addEventListener('DOMContentLoaded', function () {
  ...
  function hashChange () {
    const currentItems = document.querySelectorAll(
      '.tocify-subheader.visible, .tocify-item.tocify-focus'
    )
    ...
    if (currentTag) {
      ...
      if (siblings) {
        Array.from(
          siblings.querySelectorAll('.tocify-subheader')
        ).forEach((elem) => {
  ...
  const langSelector = document.querySelectorAll(
    '.lang-selector button.lang-button'
  )
  ...
  function setActiveLanguage (newLanguage) {
    ...
    Array.from(langSelector).forEach((elem) => {
      elem.classList.toggle(
        'active',
        elem.getAttribute('data-language-name') === newLanguage
      )
    })
