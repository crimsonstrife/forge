...
function tryItOut (endpointId) {
  ...
  document
    .querySelectorAll(
            `input[data-endpoint=${endpointId}],label[data-endpoint=${endpointId}]`
    )
  ...
  document
    .querySelectorAll(`#form-${endpointId} details`)
...
function handleResponse (endpointId, response, status, headers) {
  ...
  const responseContentEl = document.querySelector(
    '#execution-response-content-' + endpointId
  )
  ...
  if (isLaravelDump) {
    responseContentEl.innerHTML =
            response === ''
              ? responseContentEl.dataset.emptyResponseText
              : response
  } else {
  ...
  const statusEl = document.querySelector(
    '#execution-response-status-' + endpointId
  )
...
function handleError (endpointId, err) {
  ...
  const $errorMessageEl = document.querySelector(
    '#execution-error-message-' + endpointId
  )
...
async function executeTryOut (endpointId, form) {
  ...
  const query = {}
  const queryParameters = form.querySelectorAll(
    'input[data-component=query]'
  )
  ...
  urlParameters.forEach(
    (el) =>
      (path = path.replace(new RegExp(`\\{${el.name}\\??}`), el.value))
  )

  const headers = Object.fromEntries(
    Array.from(form.querySelectorAll('input[data-component=header]')).map(
      (el) => [el.name, el.value]
    )
  )
