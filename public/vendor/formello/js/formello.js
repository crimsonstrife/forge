document.addEventListener("DOMContentLoaded", function () {
    ...
    maskElements.forEach(function (el) {
        try {
            const maskOptions = JSON.parse(
                el.getAttribute("data-formello-mask"),
            );
    ...
    const datepickerElements = document.querySelectorAll(
        "[data-formello-datepicker]",
    );
    datepickerElements.forEach(function (el) {
        try {
            const flatpickrOptions = JSON.parse(
                el.getAttribute("data-formello-datepicker"),
            );
    ...
    const colorpickerElements = document.querySelectorAll(
        "[data-formello-colorpicker]",
    );
    colorpickerElements.forEach(function (el) {
        try {
            const pickrOptions = JSON.parse(
                el.getAttribute("data-formello-colorpicker"),
            );
    ...
    const wysiwygElements = document.querySelectorAll(
        "[data-formello-wysiwyg]",
    );
    if (wysiwygElements.length > 0 && typeof Jodit !== "undefined") {
        wysiwygElements.forEach(function (el) {
            try {
                const joditOptions = JSON.parse(
                    el.getAttribute("data-formello-wysiwyg"),
                );
        ...
        console.warn(
            "Jodit not loaded but WYSIWYG elements found. Make sure to include Jodit script before formello.js",
        );
    }

    // Select2 initialization
    const select2Elements = document.querySelectorAll(
        "[data-formello-select2]",
    );
    if (typeof $ !== "undefined" && $.fn.select2) {
        ...
        console.warn(
            "Select2 not loaded but Select2 elements found. Make sure to include Select2 script and jQuery before formello.js",
        );
    }
});
