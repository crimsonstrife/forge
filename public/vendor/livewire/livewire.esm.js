        ...
        function src_default(Alpine23) {
            ...
            function collapse3(el, { modifiers }) {
                ...
                const setFunction = (el2, styles) => {
                    const revertFunction = Alpine23.setStyles(el2, styles);
...
const require_module_cjs4 = __commonJS({
    "../alpine/packages/persist/dist/module.cjs.js"(exports, module) {
        ...
        function src_default(Alpine23) {
            ...
            Object.defineProperty(Alpine23, "$persist", {
                get: () => persist3(),
            });
...
wireProperty("$js", (component) => {
    const fn = component.addJsAction.bind(component);
    const jsActions = component.getJsActions();
...
// js/index.js
const import_alpinejs21 = __toESM(require_module_cjs());
...
import_alpinejs7.default.magic("js", (el) => {
    const component = closestComponent(el);
    ...
    for (let i = 0; i < el.attributes.length; i++) {
        if (el.attributes[i].name.startsWith("wire:show")) {
            import_alpinejs11.default.bind(el, {
                [directive2.rawName.replace("wire:transition", "x-transition")]:
...
import_alpinejs15.default.interceptInit((el) => {
    if (el.hasAttribute("wire:cloak")) {
        import_alpinejs15.default.mutateDom(() =>
...
const import_alpinejs19 = __toESM(require_module_cjs());
import_alpinejs19.default.interceptInit((el) => {
    for (let i = 0; i < el.attributes.length; i++) {
        if (el.attributes[i].name.startsWith("wire:show")) {
            const { name, value } = el.attributes[i];
            const modifierString = name.split("wire:show")[1];
...
const import_alpinejs20 = __toESM(require_module_cjs());
import_alpinejs20.default.interceptInit((el) => {
    for (let i = 0; i < el.attributes.length; i++) {
        if (el.attributes[i].name.startsWith("wire:text")) {
            const { name, value } = el.attributes[i];
            const modifierString = name.split("wire:text")[1];
            const expression = value.startsWith("!")
                ? "!$wire." + value.slice(1).trim()
                : "$wire." + value.trim();
...
const export_Alpine = import_alpinejs21.default;
