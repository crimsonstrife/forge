    ...
    s === void 0 && (s = {}),
    {
      ...
      async fn (t) {
        ...
        if (
          ...
          let V, G
          const E =
                        (((V = o.flip) == null ? void 0 : V.index) || 0) + 1
          const J = Y[E]
          if (
            J &&
                        (!(d === 'alignment' ? b !== B(J) : !1) ||
...
function N (s) {
  ...
  const t = s.assignedSlot || s.parentNode || (Tt(s) && s.host) || P(s)
...
function rt (s, t, e, i) {
  ...
  if (o) {
    ...
    for (; m && i && f !== p;) {
      ...
      const O = w.top + (m.clientTop + parseFloat(b.paddingTop)) * g.y;
      ((a *= g.x),
      (c *= g.y),
      (h *= g.x),
      (d *= g.y),
...
function ye (s, t) {
  ...
  if (c <= 0) {
    ...
    const f =
            ...
            0
    const p = Math.abs(i.clientWidth - d.clientWidth - f)
    p <= Ft && (o -= p)
  } else c <= Ft && (o += c)
  ...
  return {
    ...
    init () {
      ...
      this.$watch('state', async (L) => {
        ...
        this.isLoading = !0
        const V = await this.$wire.updateTableColumnState(
          f,
          v,
          this.state
        );
