(function () {
    const uuid = () =>
        'ai-' + ([1e7] + -1e3 + -4e3 + -8e3 + -1e11).replace(/[018]/g, c =>
    ...
    tinymce.PluginManager.add('action-items', function (editor) {
        ...
        const toggle = (li) => setChecked(li, li.getAttribute('data-ai-checked') !== 'true');

        // Normalize
        const normalize = () => {
            editor.getBody().querySelectorAll(liSel).forEach(li => {
                // label -> span
                const oldLabel = li.querySelector('label.ai-item');
                if (oldLabel) {
                    const span = editor.getDoc().createElement('span');
                    span.className = 'ai-item';
                    while (oldLabel.firstChild) { span.appendChild(oldLabel.firstChild); }
                ...
                if (!text) {
                    ...
                    const nodesToMove = [];
                    itemWrap.childNodes.forEach(n => { if (n !== cb) { nodesToMove.push(n); } });
                    nodesToMove.forEach(n => text.appendChild(n));
                    itemWrap.appendChild(text);
                }

                // sync checked attrs
                const checked = li.getAttribute('data-ai-checked') === 'true';
                cb.checked = checked;
        ...
        editor.on('keyup', (e) => { if (e.key === 'Enter') { normalize(); } });

        // Mouse: toggle on checkbox click
        editor.on('mousedown', (e) => {
            const cb = e.target && e.target.closest && e.target.closest('input[type="checkbox"]');
            if (!cb) { return; }
            const li = cb.closest(liSel);
            if (!li || !editor.getBody().contains(li)) { return; }
            e.preventDefault();
            toggle(li);
        });
