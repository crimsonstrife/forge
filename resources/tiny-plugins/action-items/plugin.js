    ...
    tinymce.PluginManager.add('action-items', function (editor) {
        ...
        editor.on('keydown', (e) => {
            if (e.key === ' ' || e.key === 'Enter') {
                const li = editor.selection?.getNode()?.closest?.('li[data-ai-id]');
                if (li) { maybeToggle(li); e.preventDefault(); }
            }
        });

        // Sync checkboxes on load
        const sync = () => {
            editor.getBody().querySelectorAll('li[data-ai-id]').forEach(li => {
                ...
                if (!cb) {
                    ...
                    cb.type = 'checkbox'; cb.setAttribute('aria-hidden', 'true');
                    (li.querySelector('label.ai-item') || li).insertBefore(cb, li.firstChild);
                }
                cb.checked = checked;
