(function () {
    tinymce.PluginManager.add('mentions-lite', function (editor) {
        ...
        function termOf(pattern) {
            ...
            if (pattern && typeof pattern === 'object') return pattern.term ?? pattern.query ?? pattern.text ?? '';
            return '';
        }
        ...
        function resolveActionArgs(a, b, c) {
            var api = null, rng = null, value = null;
            ...
            for (var i = 0; i < 3; i++) {
                ...
                if (x && typeof x === 'object') {
                    ...
                    if ((typeof x.hide === 'function') || (typeof x.replace === 'function')) { api = x; continue; }
                    // Range-like object
                    if (x.startContainer || x.commonAncestorContainer || x.nativeRange) { rng = x; continue; }
        ...
        function insertViaApiOrEditor(api, rng, html) {
            ...
            if (rng && editor.selection && typeof editor.selection.setRng === 'function') editor.selection.setRng(rng);
        ...
        function renderCard(_api, data) {
            ...
            el.innerHTML = '<div><strong>' + esc(v.label || (data && data.text) || '') +
                '</strong><small>' + esc(v.sublabel || '') + '</small></div>';
        ...
        editor.ui.registry.addAutocompleter('mentionsUsers', {
            ...
            fetch: async function (pattern) {
                ...
                for (var i = 0; i < list.length; i++) {
                    var u = list[i];
                    out.push({ type: 'menuitem', text: u.label, value: JSON.stringify(u) });
                }
                return out;
            },
            onAction: function (a, b, c) {
                var resolved = resolveActionArgs(a, b, c);
                var v = toObj(resolved.value);
                var html = '<a href="' + v.url + '" data-mention-type="user" data-mention-id="' + esc(v.id) + '">@' + esc(v.label) + '</a>&nbsp;';
        ...
        editor.ui.registry.addAutocompleter('mentionsIssues', {
            ...
            onAction: function (a, b, c) {
                var resolved = resolveActionArgs(a, b, c);
                var v = toObj(resolved.value);
                var html = '<a href="' + v.url + '" data-mention-type="issue" data-mention-id="' + esc(v.id) + '">#' + esc(v.label) + '</a>&nbsp;';
                insertViaApiOrEditor(resolved.api, resolved.rng, html);
            },
            itemRenderer: renderCard
        });
