    ...
    document.querySelectorAll('[data-kanban-list]').forEach((el) => {
        Sortable.create(el, {
            ...
            onEnd: (evt) => {
                ...
                const orderedIssueIds = [...evt.to.querySelectorAll('[data-issue-id]')].map(li => Number(li.dataset.issueId));
                ...
                if (window.Livewire && typeof window.Livewire.find === 'function') {
                    ...
                    const comp = Object.values(window.Livewire.components.componentsById || {})[0];
                    if (comp) {
