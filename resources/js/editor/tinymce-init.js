...
(function () {
    ...
    function tinyEditor(cfg) {
        ...
        ['change','undo','redo','SetContent'].forEach(evt => {
            editor.on(evt, () => { clearTimeout(syncTimer); syncBack(); });
        });
        ...
        return {
            async init() {
                ...
                await window.tinymce.init({
                    ...
                    setup: (ed) => {
                        ...
                        // Keep data in sync
                        const syncEvents = ['change', 'input', 'undo', 'redo', 'keyup', 'SetContent'];
            ...
            destroy() {
                try { window.tinymce?.remove?.(selector); } catch (_) {}
                editor = null;
            },
        };
    }
    ...
    function initAll(selector = 'textarea[data-tiny]') {
        document.querySelectorAll(selector).forEach(async (el) => {
            ...
            const inst = tinyEditor({
                ...
                toolbar: el.dataset.toolbar || 'undo redo | bold italic link | bullist numlist | checklist mentionUser mentionIssue | code',
