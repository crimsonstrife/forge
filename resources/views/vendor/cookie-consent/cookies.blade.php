            ...
            <div class="cookies__wrapper">
                ...
                <div class="cookies__actions">
                    @cookieconsentbutton(
                        ...
                        label: __('cookieConsent::cookies.essentials'),
                        attributes: ['class' => 'cookiesBtn cookiesBtn--essentials']
                    )
                    @cookieconsentbutton(
                        action: 'accept.all',
                        label: __('cookieConsent::cookies.all'),
                        attributes: ['class' => 'cookiesBtn cookiesBtn--accept']
                    )
                </div>
            </div>
        </div>
        ...
        <div class="cookies__expandable cookies__expandable--custom" id="cookies-policy-customize">
            <form action="{{ route('cookieconsent.accept.configuration') }}" method="post" class="cookies__customize">
                            ...
                            <label for="cookies-policy-check-{{ $category->key() }}" class="cookies__category">
                                @if ($category->key() === 'essentials')
                                    ...
                                    <input type="checkbox" name="categories[]" value="{{ $category->key() }}"
                                           id="cookies-policy-check-{{ $category->key() }}" checked disabled />
                                @else
                                    <input type="checkbox" name="categories[]" value="{{ $category->key() }}"
                ...
                <div class="cookies__save">
                    <wa-button type="submit" variant="brand" href="#cookies-policy-customize">@lang('cookieConsent::cookies.save')</wa-button>
                </div>
            </form>
        </div>
    </div>
