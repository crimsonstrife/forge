<div>
    ...
    <button type="button" onclick="toggleIconPicker()" class="px-4 py-2 bg-gray-500 text-white rounded">
        Select Icon
    </button>

    <!-- Modal with the icon picker -->
    <div id="icon-picker-modal" class="fixed z-10 inset-0 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true" style="display: none;">
        <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true"></div>

            <!-- Modal content -->
            <div id="icon-modal-content" class="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-4xl sm:w-full min-w-[600px] min-h-[400px]">
                <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <!-- Grid of icons with responsive columns -->
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 max-h-96 overflow-y-auto">
                        @foreach ($getIcons() as $icon)
                            <div onclick="selectIcon('{{ $icon->id }}')" class="cursor-pointer border p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 icon-picker-button">
...
<style>
    ...
    #icon-picker-modal #icon-modal-content {
        width: 40vw; /* Set the width to 40% of the viewport */
        max-width: 600px; /* Set a maximum width */
    }

    /* Ensure the modal is a decent height, but not too tall */
    #icon-picker-modal #icon-modal-content {
        height: 60vh; /* Set the height to 60% of the viewport */
        max-height: 400px; /* Set a maximum height */
    }

    /* Keep the modal from being too high and clipping into the header */
    #icon-picker-modal #icon-modal-content {
        top: 20vh; /* Set the top to 20% of the viewport */
        position: sticky; /* Keep the modal in place */
    }

    /* Set the SVG size and color */
    #icon-picker-modal svg {
        min-width: 16px; /* Ensure a minimum width */
        min-height: 16px; /* Ensure a minimum height */
        max-width: 32px; /* Ensure the SVG scales */
        max-height: 32px; /* Ensure the SVG scales */
        width: 100%; /* Ensure the SVG scales */
        height: 100%; /* Ensure the SVG scales */
    ...
    #icon-picker-modal .grid {
        display: grid;
        grid-template-columns: repeat(auto-fill, minmax(50px, 1fr)); /* Ensure responsive columns */
        gap: 1rem;
    }

    /* Modal background and grid behavior */
    #icon-picker-modal .grid {
        max-height: 400px; /* Set max height for the grid */
        overflow-y: auto; /* Enable vertical scrolling for icons if needed */
    }
    ...
    #icon-picker-modal .icon-picker-button {
        ...
        justify-content: center;
        align-items: center;
        border-style: none; /* Remove any border */
    }
