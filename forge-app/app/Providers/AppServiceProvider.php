...
class AppServiceProvider extends ServiceProvider
{
    ...
    {
        // Check if the database exists, and if so, do any of the GeneralSettings exist in the settings table?
        // If not, do not attempt to load the settings or Spatie will throw an error.
        $dbExists = false;
        try {
            $dbExists = $this->app->make('db')->connection()->DB::table('settings')->exists(); // Check if the settings table exists

            // If the settings table exists, check if any of the GeneralSettings exist before loading them
            if ($dbExists) {
                try {
            ...
            // If the environment is local, throw a warning but do nothing, otherwise, throw an error
            if ($this->app->environment('local')) {
                $this->app->make('log')->warning('Database not found or settings table not found.');
            } else {
                throw $e;
            }
        }
